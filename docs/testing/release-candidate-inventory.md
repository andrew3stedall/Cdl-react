...
| Preferences | `preferences_router` | **Partial and not currently promoted in primary navigation.** PostgreSQL repository wiring exists, but the router still uses a fixed demo user identity. | No release-candidate browser journey is claimed. | Replace the fixed demo identity with the authenticated session boundary before exposing preferences as a primary workflow. |
...
1. Replace the preferences router's fixed demo identity with the authenticated session boundary before preferences are treated as a supported product workflow.
