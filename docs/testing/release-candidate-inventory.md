...
| Squad management, transfers and trades | `squad_router`, `movement_router` | **Partial.** PostgreSQL squad ownership and synthetic historical projection are proven. The current interest mutation remains an explicitly preview-only browser contract backed by local storage rather than an authenticated API write. | A focused mobile and desktop journey proves an interest mutation survives reload, duplicate submission returns explicit feedback, and the invalid duplicate leaves persisted preview state unchanged. | Add an authenticated API-backed squad mutation with authorization and PostgreSQL browser integration; do not treat local-storage evidence as server persistence. |
| Team selection | `team_selection_router`, `modernisation_weekly_router` | **Proven for the deterministic browser contract.** Selection loading, lineup validation, lineup persistence, chip mutation, fixture locks and session boundaries have focused automated evidence. | The browser suite proves invalid-lineup feedback, a valid lineup save, lineup and wildcard state surviving reload, and disabled lineup/chip controls when locked at both mobile and desktop widths. | Live PostgreSQL browser integration and staging identity remain separate gates; captaincy-specific mutation coverage is not isolated as its own journey. |
...
| `App Screenshots` | Builds the frontend, runs primary browser interactions plus the focused squad preview mutation journey, responsive/accessibility checks, captures screenshots and uploads artifacts. | Uses repository-safe test data; squad interest persistence is local-storage preview evidence, not an authenticated API or live staging acceptance pass. |
...
1. Authenticated API-backed squad mutation persistence and authorization evidence beyond the preview-only local-storage journey.
