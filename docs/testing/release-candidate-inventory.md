...
| Team selection | `team_selection_router`, `modernisation_weekly_router` | **Partial.** Selection contracts and validation exist, but captaincy, chips, locks and persistence-across-reload are not yet mapped to focused release evidence. | Representative team-selection interactions are covered. | Add one release test matrix for captaincy, chips, lock enforcement, validation errors and reload persistence. |
