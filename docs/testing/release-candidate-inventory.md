...
| Team selection | `team_selection_router`, `modernisation_weekly_router` | **Proven for the deterministic browser contract.** Selection loading, the full player profile opened as a full-screen replacement layer, formation-aware swaps from Starting XI/Bench/Reserves, the two-player latest-four-fixture review drawer, lineup persistence, chip mutation, fixture locks and session boundaries have focused automated evidence. | The browser suite proves that list view has no player movement dropdowns, the selected player opens the full profile layer with its action toolbar and shirt token, the layer covers the mobile navigation, only legal swap candidates are highlighted, the review drawer presents both players and confirm/cancel actions, the staged swap persists through Save lineup, and lineup/chip/profile substitution controls are disabled when a fixture lock is active. The journey runs at both mobile and desktop widths. | Live PostgreSQL browser integration and staging identity remain separate gates; captaincy-specific mutation coverage is not isolated as its own journey. |
