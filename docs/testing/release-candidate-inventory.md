...
| `App Screenshots` | Builds the frontend, runs primary browser interactions plus the focused authenticated squad-interest journey, responsive/accessibility checks, captures screenshots and uploads artifacts. | Uses a deterministic API test double for browser state. It proves frontend request/rejection behaviour, while PostgreSQL persistence is independently proved in backend CI; it is not live staging acceptance. |
