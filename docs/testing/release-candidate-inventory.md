...
| Squad management, transfers and trades | `squad_router`, `movement_router` | **Proven for authenticated interest and trade proposal mutations in deterministic PostgreSQL CI.** Interest and trade list/create routes require an authenticated manager session. PostgreSQL mode persists both mutation types, reloads them through repository reads, and rejects invalid writes before changing stored rows. | Focused mobile and desktop journeys prove interest and trade creation, reload from server state, rejected duplicate or invalid requests, explicit unauthorized feedback, and unchanged displayed state after rejection. The browser uses a deterministic API test double; PostgreSQL persistence is proved separately by backend CI. | Trade acceptance/rejection still lacks counterparty-specific authorization and transition-conflict evidence. Live browser-to-PostgreSQL and staging identity remain separate gates. |
...
| `Backend PostgreSQL` | Applies all Alembic migrations to a clean PostgreSQL service and runs focused release paths for league, dashboard, FDR, authenticated squad interests and trades, and historical import. | Local CI database evidence, not live staging evidence. |
...
1. Counterparty-specific authorization and valid transition rules for trade acceptance/rejection, including stale-state conflict evidence.
