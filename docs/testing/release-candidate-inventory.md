...
| Team selection | `team_selection_router`, `modernisation_weekly_router` | **Proven for the deterministic browser contract.** Selection loading, the full player profile opened as a full-screen replacement layer, formation-aware substitutions from that profile, numbered bench ordering, lineup persistence, chip mutation, fixture locks and session boundaries have focused automated evidence. | The browser suite proves that list view has no player movement dropdowns, the selected player opens the full profile layer with its action toolbar and shirt token, the layer covers the mobile navigation, only legal substitution candidates are shown, a numbered bench slot can be chosen, the profile substitution save survives reload, and lineup/chip/profile substitution controls are disabled when a fixture lock is active. The journey runs at both mobile and desktop widths. | Live PostgreSQL browser integration and staging identity remain separate gates; captaincy-specific mutation coverage is not isolated as its own journey. |
