...
| Squad management, transfers and trades | `squad_router`, `movement_router` | **Partial.** PostgreSQL squad ownership and synthetic historical projection are proven, while every mutation workflow has not yet been inventoried against reload and authorization evidence. | Player search, interest mutation, player detail and sample trade creation are covered. | Add focused persistence, invalid-action and authorization evidence for squad mutations. |
...
1. Focused persistence, invalid-action and authorization evidence for squad mutations.
