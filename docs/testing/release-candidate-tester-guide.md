...
| Team selection | List view has no player movement dropdowns; selecting a player opens the full profile as a full-screen layer over navigation with visible squad-management actions and the Squad shirt token. **Sub** is available from Starting XI, Bench and Reserves; it closes the profile, highlights formation-valid candidates, opens a two-column latest-four-fixture review drawer for the selected candidate, and stages the swap until **Save lineup**. | Live browser-to-PostgreSQL integration remains a staging gate. |
