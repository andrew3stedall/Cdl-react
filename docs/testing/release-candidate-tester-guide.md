...
| Team selection | List view has no player movement dropdowns; the player context menu shows only formation-valid substitutions, supports a numbered bench slot, and a valid save shows `Lineup saved and validated.`; lineup and wildcard state survive reload. | Live browser-to-PostgreSQL integration remains a staging gate. |
