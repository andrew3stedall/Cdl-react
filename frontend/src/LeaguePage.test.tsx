...
import { shouldShowFixturePoints } from './components/fixture/FixtureSquadComparison';
import type { FixtureDetailResponse, FixtureSquad, FixtureSquadPlayer, LeagueClient, LeagueSnapshot } from './league-api';
...
describe('LeaguePage', () => {
  ...
  test('opens a completed or live fixture player with their points breakdown', async () => {
    const { container, root } = await renderPage();
  ...
  test('only reveals current-gameweek points after the player kickoff', () => {
    const player: FixtureSquadPlayer = {
      id: 'current-player',
      ...
      form: 5,
      slot: 'starter',
      fixtureFixtures: [{ fixtureId: 'fixture-current', gameweek: 12, opponent: drafton, isHome: true, kickoffAt: '2026-08-29T10:00:00Z' }],
    };
