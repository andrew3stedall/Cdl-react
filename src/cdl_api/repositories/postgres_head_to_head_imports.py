...
class PostgreSQLHistoricalHeadToHeadImportRepository(PostgreSQLHistoricalImportRepository):
    """Project matchup aggregates only from mapped persisted fixtures and results."""

    def run(self, batch: HistoricalImportBatch, *, dry_run: bool) -> HistoricalImportAudit:
        if any(record.entity_type != "head_to_head_record" for record in batch.records):
            raise ValueError("Historical head-to-head projection accepts only head_to_head_record records.")
        ...
        with self._session_factory() as session:
            ...
            for record in batch.records:
                fixture_keys = [str(value) for value in record.payload.get("fixture_source_keys", [])]
                ...
                if record_id is None or any(value is None for value in fixture_ids):
                    review_items.append(record.source_record_id)
                ...
                aggregate = self._aggregate(session, [str(value) for value in fixture_ids], team.id, opponent.id)
                ...
                expected_aggregate = {
                    ...
                    for key in ("played", "wins", "draws", "losses", "points_for", "points_against")
                }
                if aggregate != expected_aggregate:
                    raise ValueError("Head-to-head aggregate does not match persisted fixture results.")
                ...
                current = self._payload(session, head_to_head_records_table, str(record_id))
                if current == expected:
                    unchanged_domain += 1
                elif current is not None:
                    raise ValueError(
                        f"Head-to-head target {record_id!r} already exists with different content."
                    )
                else:
                    projected += 1
            ...
            self._persist_reviews(session, batch, conflicts, review_reasons)
    ...
    def _aggregate(
        ...
        aggregate = {"played": 0, "wins": 0, "draws": 0, "losses": 0, "points_for": 0, "points_against": 0}
        for fixture_id in fixture_ids:
            ...
            home_id = str(dict(fixture).get("home_team", {}).get("id", ""))
            away_id = str(dict(fixture).get("away_team", {}).get("id", ""))
            if {home_id, away_id} != {team_id, opponent_id}:
                raise ValueError("Head-to-head fixture teams do not match the aggregate teams.")
            home_score = int(dict(result).get("home_score", 0))
            away_score = int(dict(result).get("away_score", 0))
    ...
    def _persist_reviews(
        ...
        for record_id, reason in sorted(review_reasons.items()):
            review_id = hashlib.sha256(f"{batch.batch_id}:review:{record_id}".encode()).hexdigest()[:64]
            session.execute(
