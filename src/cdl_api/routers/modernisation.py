...
LEAGUE: dict[str, Any] = {
    "id": "league-cdl",
    "name": "Castle Draft League",
    "default_manager_count": 8,
}
SEASON: dict[str, Any] = {
    "id": "season-2026",
    "league_id": "league-cdl",
    "name": "2026/27",
    "status": "setup",
    "active_rule_version_id": "rule-version-2026-draft",
}
TEAMS = [
    {"id": "team-castle", "league_id": "league-cdl", "name": "Castle FC"},
    {"id": "team-drafton", "league_id": "league-cdl", "name": "Drafton Athletic"},
]
SEASON_TEAMS = [
    {
        "id": "season-team-castle",
        "team_id": "team-castle",
        "manager_membership_id": "membership-castle",
    },
    {
        "id": "season-team-drafton",
        "team_id": "team-drafton",
        "manager_membership_id": "membership-drafton",
    },
]
MEMBERSHIPS = [
    {"id": "membership-commissioner", "role": "commissioner"},
    {"id": "membership-vice", "role": "vice_commissioner"},
    {"id": "membership-castle", "role": "manager"},
    {"id": "membership-drafton", "role": "manager"},
]
STATUS_HISTORY = [
    {
        "from_status": None,
...
RULE_VERSIONS = [
    {
        "id": "rule-version-2026-draft",
        "status": "draft",
        "version": 1,
        "config_json": {
            "league": {"max_managers": 8},
            "draft": {"squad_size": 3},
        },
    }
]
APPROVALS = [
    {
        "id": "approval-transfer-1",
        "status": "pending",
        "required_approver_role": "vice_commissioner",
        "basis": "commissioner_involved",
        "involved_membership_ids": ["membership-commissioner", "membership-castle"],
        "actual_approver_membership_id": None,
    }
]
...
FPL_FETCH_COUNTER = 1
PLAYERS = [
    {"id": "fpl-101", "display_name": "Alex Keeper", "value": 5.0},
    {"id": "fpl-102", "display_name": "Ben Defender", "value": 6.0},
    {"id": "fpl-103", "display_name": "Casey Midfielder", "value": 7.5},
    {"id": "fpl-104", "display_name": "Riley Forward", "value": 9.0},
]
DRAFT_ROOM: dict[str, Any] = {
    "id": "draft-season-2026",
    "status": "live",
    "pick_order": ["season-team-drafton", "season-team-castle"],
    "current_pick_number": 1,
    "preselection_queue": {"season-team-drafton": ["fpl-103"]},
    "picks": [],
    "events": [],
}
SQUAD_CAP = 3
ASSIGNMENTS = [
    {
        ...
        "fpl_player_id": "fpl-101",
        "start_gameweek_id": "gw-1",
        "end_gameweek_id": None,
        "start_reason": "seeded_squad",
        "end_reason": None,
    },
    {
        "id": "assignment-102",
        "season_team_id": "season-team-castle",
        "fpl_player_id": "fpl-102",
        "start_gameweek_id": "gw-1",
        "end_gameweek_id": None,
        "start_reason": "seeded_squad",
        "end_reason": None,
...
def _active(season_team_id: str | None = None) -> list[dict[str, Any]]:
    ...
    if season_team_id is None:
        return assignments
    return [item for item in assignments if item["season_team_id"] == season_team_id]
...
def rule_versions() -> dict[str, Any]:
    return {
        ...
        "active_rule_version_id": SEASON["active_rule_version_id"],
    }


@router.post("/rule-versions/validate", response_model=None)
def validate_rules(payload: RuleValidationRequest) -> dict[str, Any]:
    league_config = payload.config_json.get("league", {})
    draft_config = payload.config_json.get("draft", {})
    issues = []

    if isinstance(league_config, dict) and int(league_config.get("max_managers", 8)) < 2:
        issues.append(
            {"field": "league.max_managers", "message": "At least two managers are required."}
        )
    if isinstance(draft_config, dict) and int(draft_config.get("squad_size", 3)) < 1:
        issues.append({"field": "draft.squad_size", "message": "Squad size must be positive."})

    return {
        "valid": not issues,
        "issues": issues,
        "rule_version_reference": SEASON["active_rule_version_id"],
        "editability_constraints": {
            "locked_after_statuses": ["draft_live", "active", "complete", "archived"]
        },
    }
...
def decide_approval(
    ...
    if _role(payload.approver_membership_id) != payload.approver_role:
        return _err(
            ErrorCode.FORBIDDEN,
            "Approver role cannot decide this request.",
            status.HTTP_403_FORBIDDEN,
        )
    if payload.approver_role != approval["required_approver_role"]:
        return _err(
            ErrorCode.FORBIDDEN,
            "Approver role cannot decide this request.",
            status.HTTP_403_FORBIDDEN,
    ...
    if payload.approver_membership_id in approval["involved_membership_ids"]:
        return _err(ErrorCode.FORBIDDEN, "Approver is conflicted.", status.HTTP_403_FORBIDDEN)
...
def refresh_fpl(payload: FplRefreshRequest) -> dict[str, Any]:
    ...
    raw_payload = {"endpoint_key": payload.endpoint_key, "sequence": FPL_FETCH_COUNTER}
    FPL_FETCH_COUNTER += 1
    source = {
        "provider": "FPL",
        "endpoint_key": payload.endpoint_key,
        "status": "fresh",
        "response_hash": _hash(raw_payload),
    ...
    return {
        "source": source,
        "fetch_log": _audit("fpl_fetch", "external_payload_cache", payload.endpoint_key),
    }
...
def draft_room() -> dict[str, Any]:
    ...
    picked_player_ids = {pick["fpl_player_id"] for pick in DRAFT_ROOM["picks"]}
    controlled_player_ids = {item["fpl_player_id"] for item in _active()}
    available = [
        player
        for player in PLAYERS
        if player["id"] not in picked_player_ids and player["id"] not in controlled_player_ids
    ]
    return {
        **DRAFT_ROOM,
        "current_season_team_id": DRAFT_ROOM["pick_order"][index],
...
def _pick(payload: DraftPickRequest) -> dict[str, Any] | JSONResponse:
    current_team_id = draft_room()["current_season_team_id"]
    can_pick_on_behalf = _role(payload.actor_membership_id) == "commissioner"
    if payload.season_team_id != current_team_id and not can_pick_on_behalf:
        return _err(
            ErrorCode.FORBIDDEN,
            "Only the current manager can pick.",
            status.HTTP_403_FORBIDDEN,
        )
    ...
    pick = {
        "id": f"pick-{uuid4().hex[:8]}",
        "pick_number": DRAFT_ROOM["current_pick_number"],
        "season_team_id": payload.season_team_id,
        "fpl_player_id": payload.fpl_player_id,
        "pick_source": payload.source,
        "seconds_taken": 24,
    }
    assignment = {
        "id": f"assignment-{uuid4().hex[:8]}",
        "season_team_id": payload.season_team_id,
        "fpl_player_id": payload.fpl_player_id,
        "start_gameweek_id": "gw-1",
        "end_gameweek_id": None,
        "start_reason": "draft_pick",
        "end_reason": None,
    }
    ...
    if payload.source == "commissioner_on_behalf":
        audit_event = _audit("commissioner_pick_on_behalf", "draft_pick", pick["id"])
        event["audit_event_id"] = audit_event["id"]
...
def autopick() -> dict[str, Any] | JSONResponse:
    ...
    available_player_ids = {player["id"] for player in room["available_players"]}
    queued = [
        player_id
        for player_id in DRAFT_ROOM["preselection_queue"].get(team_id, [])
        if player_id in available_player_ids
    ]
    if queued:
        player_id = queued[0]
        source = "manager_preselection_auto"
    else:
        player_id = max(room["available_players"], key=lambda player: player["value"])["id"]
...
def active_squad(season_team_id: str = "season-team-castle") -> dict[str, Any]:
    ...
    return {
        ...
        "empty_slots": max(SQUAD_CAP - len(assignments), 0),
        "temporary_rights": [
            right
            for right in RIGHTS
            if right["season_team_id"] == season_team_id and right["status"] == "pending"
        ],
    }
...
def squad_history(season_team_id: str = "season-team-castle") -> dict[str, Any]:
    return {
        "season_team_id": season_team_id,
        "assignments": [item for item in ASSIGNMENTS if item["season_team_id"] == season_team_id],
    }
...
def activate_right(right_id: str) -> dict[str, Any] | JSONResponse:
    ...
    if right["status"] != "pending":
        return _err(ErrorCode.CONFLICT, "Temporary right is not pending.", status.HTTP_409_CONFLICT)
    ...
    assignment = {
        "id": f"assignment-{uuid4().hex[:8]}",
        "season_team_id": right["season_team_id"],
        "fpl_player_id": right["fpl_player_id"],
        "start_gameweek_id": right["gameweek_id"],
        "end_gameweek_id": None,
        "start_reason": "temporary_right_activation",
        "end_reason": None,
    }
    ASSIGNMENTS.append(assignment)
    return {
        ...
        "active_squad": active_squad(str(right["season_team_id"])),
    }
...
def remove_player(payload: RemovePlayerRequest) -> dict[str, Any] | JSONResponse:
    assignment = next(
        (
            ...
            if item["season_team_id"] == payload.season_team_id
            and item["fpl_player_id"] == payload.fpl_player_id
            and item["end_gameweek_id"] is None
        ),
        None,
    )
    if assignment is None:
        return _err(
            ErrorCode.NOT_FOUND,
            "Active squad assignment not found.",
...
def player_availability(fpl_player_id: str) -> dict[str, Any]:
    ...
    if assignment is not None:
        return {
            ...
            "available": False,
            "reason": "active_squad_assignment",
            "controlled_by_season_team_id": assignment["season_team_id"],
        }
    ...
    right = next(
        (
            item
            for item in RIGHTS
            if item["fpl_player_id"] == fpl_player_id and item["status"] == "pending"
        ),
        None,
    )
    if right is not None:
        return {
            "fpl_player_id": fpl_player_id,
            "available": False,
            "reason": "temporary_right_pending",
            "controlled_by_season_team_id": right["season_team_id"],
        }
