...
def test_plan_summary_allows_refresh_only_drift_on_a_no_change_plan() -> None:
    plan = _plan(
        ...
        resource_drift=[
            {
                "address": "google_monitoring_alert_policy.cloud_sql_error_logs",
                "type": "google_monitoring_alert_policy",
                "change": {
                    "actions": ["update"],
                    "before": {"provider_normalized_field": "before"},
                    "after": {"provider_normalized_field": "after"},
                },
            }
        ],
    )
    ...
    assert exit_code == 0
    assert "PASS: refresh-only drift reported; no managed changes" in summary
    assert "google_monitoring_alert_policy.cloud_sql_error_logs" in summary
