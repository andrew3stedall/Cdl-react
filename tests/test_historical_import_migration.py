...
def test_clean_postgres_has_historical_import_foundation() -> None:
    ...
    batch_uniques = {constraint["name"] for constraint in inspector.get_unique_constraints("historical_import_batches")}
    payload_uniques = {constraint["name"] for constraint in inspector.get_unique_constraints("historical_source_payloads")}
    mapping_uniques = {constraint["name"] for constraint in inspector.get_unique_constraints("historical_source_mappings")}
